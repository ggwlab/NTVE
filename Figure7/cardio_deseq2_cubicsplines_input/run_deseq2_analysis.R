...
cat("  Dispersions estimated (outlierSD = 10 for time-course)\n\n")
...
# Capture package/runtime versions for reproducibility.
writeLines(capture.output(sessionInfo()), file.path(output_dir, "sessionInfo.txt"))
installed <- installed.packages()[, c("Package", "Version")]
write.csv(installed, file.path(output_dir, "installed_packages.csv"), row.names = FALSE)
cat("  Reproducibility metadata: sessionInfo.txt, installed_packages.csv\n")
