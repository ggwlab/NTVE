"""
Supplementary Figure 13a — Secondary benchmarking transcript-length correlation
Notebook-faithful standalone reproduction of the matched, non-bootstrap Spam panel
from Suppl12_13/correlation_by_transcript_length_noMT_1000bp.ipynb.
Outputs to: refactoring_roadmap/suppl13a_plots/
"""
...
def parse_sample_names_transcript(tpm_df, lims_mapping=None):
    ...
    for col_name in tpm_df.columns:
        ...
        # Elo pattern left for completeness, unused here
        match = re.match(r"SN_S(\d+)_(\d+)", col_str)
        if match:
            sample_name_mapping[col_str] = {"Sample": int(match.group(1)), "Replicate": int(match.group(2)), "Type": "SN", "Experiment": "Elo", "ColumnName": col_str}
            continue
        match = re.match(r"Lysate_S(\d+)_(\d+)", col_str)
        if match:
            sample_name_mapping[col_str] = {"Sample": int(match.group(1)), "Replicate": int(match.group(2)), "Type": "Lysate", "Experiment": "Elo", "ColumnName": col_str}
...
output_dir_candidates = [
    get_generated_figure2_loaded_dir(),
]
output_dir = next(
    (d for d in output_dir_candidates if (d / "transcript_tpm_matrix.csv").exists() and (d / "transcript_info.csv").exists()),
    None,
)
if output_dir is None:
    raise FileNotFoundError(
        "Could not find transcript-level Spam inputs in any expected generated directory: "
        + ", ".join(str(d) for d in output_dir_candidates)
    )

print("Loading transcript-level data...")
print(f"Using loaded_data from: {output_dir}")
project_1716_tpm = pd.read_csv(output_dir / "transcript_tpm_matrix.csv", index_col=0)
project_1716_transcript_info = pd.read_csv(output_dir / "transcript_info.csv")
print(f"Project 1716 (Spam): {project_1716_tpm.shape[0]} transcripts, {project_1716_tpm.shape[1]} samples")

lims_df = pd.read_csv(ROOT / "resources" / "Project_1716_lims_simplified_Spam2_without_missing.csv")
if "Sample_Name" in lims_df.columns and "Sample_NameLIMS" in lims_df.columns:
    # Notebook-faithful mapping: RNA-seq column ID -> logical sample label like SN_1_1 / L_1_1
    lims_mapping = dict(zip(lims_df["Sample_Name"], lims_df["Sample_NameLIMS"]))
else:
    raise ValueError(
        "Expected Sample_Name and Sample_NameLIMS columns in Project_1716 LIMS mapping file"
    )

spam_biotype_dict = create_biotype_dicts(project_1716_transcript_info, gtf_df_data)
print(f"Spam protein_coding transcripts: {sum(1 for b in spam_biotype_dict.values() if b == 'protein_coding')}")

spam_sample_map = parse_sample_names_transcript(project_1716_tpm, lims_mapping)
print(f"Mapped {len(spam_sample_map)} Spam samples")
spam_organized = organize_samples_by_type_and_number(spam_sample_map, project_1716_tpm)
print("Sample organization complete.")

print("\nAnalyzing Spam experiment (matched replicates)...")
spam_corr_results_matched, spam_bins_matched = analyze_correlation_by_length(
    project_1716_tpm,
    spam_sample_map,
    project_1716_transcript_info,
    spam_biotype_dict,
    spam_organized,
    "SN",
    "L",
...
PANEL_A_SAMPLES = [1, 3, 5, 6]  # NTVE, HIV:sPAM NC, mini-gag:PABP, NLuc
...
fig_spam_corr_matched = plot_correlation_by_length(
    ...
    "SN",
    "L",
    save_filename="suppl13a_secondary_benchmarking_correlation_by_length_matched.svg",
)
plt.close(fig_spam_corr_matched)
print("Done.")
